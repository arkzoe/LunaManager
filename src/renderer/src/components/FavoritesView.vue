...
<template>
  <div class="h-full flex flex-col">
    ...
    <template v-if="viewMode === 'collections'">
      ...
      <div class="flex flex-wrap gap-4 overflow-y-auto overflow-x-hidden pr-2 flex-1 content-start">
        <div
          ...
          :key="collection.id"
          class="collection-card w-70 h-20 flex items-center gap-4 p-4 bg-bg-primary border border-border rounded-xl cursor-pointer transition-all duration-250 flex-shrink-0 box-border hover:border-border-medium hover:-translate-y-0.5"
    ...
    <template v-else>
      ...
      >
        <div
          ...
          :key="game.id"
          class="game-card w-35 h-57 bg-bg-primary rounded-xl border border-border overflow-hidden cursor-pointer transition-all duration-250 flex-shrink-0 hover:-translate-y-1 hover:border-border-medium"
    ...
    >
      <div class="w-90% max-w-100 bg-bg-primary rounded-2xl p-6 border border-border">
        <h3 class="text-lg font-semibold text-text-primary m-0 mb-4">新建收藏夹</h3>
        <input
          v-model="newCollectionName"
          type="text"
          placeholder="输入收藏夹名称"
          class="w-full h-11 px-4 bg-bg-secondary border border-border rounded-xl text-text-primary text-15px mb-5 transition-all duration-200 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
